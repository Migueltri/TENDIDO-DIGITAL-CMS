                    ...
                    <tr key={article.id} className={`hover:bg-gray-50 group ${viewMode === 'history' ? 'bg-gray-50/50 grayscale-[20%]' : ''}`}>
                      ...
                      <td className="p-4 text-right align-top pt-4">
                        <div className="flex items-center justify-end gap-2">
                          {viewMode === 'active' && (
                              <>
                                <button onClick={(e) => { e.stopPropagation(); handleTogglePin(article); }} className={`p-2 rounded-lg transition-colors border ${article.isPinned ? 'text-white bg-blue-600 border-blue-600 hover:bg-blue-700' : 'text-blue-600 bg-blue-50 border-blue-100 hover:bg-blue-100'}`} title={article.isPinned ? 'Desfijar' : 'Fijar'}><Pin size={4} /></button>
