                        ...
                        <div key={article.id} className={`p-4 flex flex-col gap-3 ${viewMode === 'history' ? 'bg-gray-50/50 grayscale-[20%]' : ''}`}>
                            <div className="flex justify-between items-start">
                                <div className="flex items-center gap-2">
                                    <img src={getCMSImageUrl(article['image'] || article.imageUrl)} className="w-12 h-12 rounded object-cover bg-gray-200" alt="" />
                                    <div>
                    ...
                    <tr key={article.id} className={`hover:bg-gray-50 group ${viewMode === 'history' ? 'bg-gray-50/50 grayscale-[20%]' : ''}`}>
                      ...
                      <td className="p-4 max-w-md">
                        <div className="flex items-start gap-3">
                          <img src={getCMSImageUrl(article['image'] || article.imageUrl)} className="w-16 h-12 rounded object-cover bg-gray-200 mt-1" alt="" />
                      ...
                      <td className="p-4 text-right align-top pt-4">
                        <div className="flex items-center justify-end gap-2">
                          {viewMode === 'active' && (
                              <>
                                <button onClick={(e) => { e.stopPropagation(); handleTogglePin(article); }} className={`p-2 rounded-lg transition-colors border ${article.isPinned ? 'text-white bg-blue-600 border-blue-600 hover:bg-blue-700' : 'text-blue-600 bg-blue-50 border-blue-100 hover:bg-blue-100'}`} title={article.isPinned ? 'Desfijar' : 'Fijar'}><Pin size={20} /></button>
