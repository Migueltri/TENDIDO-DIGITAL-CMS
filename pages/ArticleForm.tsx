  ...
  useEffect(() => {
    ...
    if (isEditMode && id) {
      ...
      if (article) {
        ...
        setFormData({
            ...article,
            contentImages: processedGallery
        });
        ...
        // 2. MODO NUEVA NOTICIA - Intentar recuperar borrador
...
const handleMainImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file && formData.authorId) {
          ...
          try {
              ...
              const updatedAuthors = authors.map(a => 
                  a.id === formData.authorId ? { ...a, imageUrl: compressed } : a
              );
  ...
  const updateGalleryCredit = (index: number, credit: string) => {
      setFormData(prev => {
          const newImages = [...(prev.contentImages || [])];
          if(newImages[index]) {
              newImages[index] = { ...newImages[index], credit };
          }
          return { ...prev, contentImages: newImages };
      });
  };
...
const handleSave = async (e: React.MouseEvent, shouldPublish: boolean) => {
    ...
    if (!formData.imageUrl) {
        alert('Por favor sube una imagen de portada para la noticia.');
        return;
    }
    ...
    if (!isValidUrl(formData.imageUrl)) {
        alert('La URL de la imagen de portada no es válida.');
        return;
    }
    ...
    try {
        if (shouldPublish) {
            ...
            if (result.success) {
                alert("✅ ¡Noticia publicada y sincronizada con éxito! Vercel está desplegando la web.");
                navigate('/noticias');
            } else {
                alert(`⚠️ Se guardó localmente, pero hubo un problema subiendo a la web: ${result.message}`);
      ...
      <form className="bg-white p-6 md:p-8 rounded-xl shadow-sm border border-gray-100 space-y-8 relative">
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red outline-none text-lg font-medium bg-white font-serif"
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            onChange={handleChange}
            rows={2}
            className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand-red/20 focus:border-brand-red outline-none bg-white font-serif leading-relaxed"
          ...
          >
             {loadingImage && (
                 <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/80">
                     ...
                     <p className="text-xs font-bold text-gray-600">Procesando imagen...</p>
                 </div>
             )}
             ...
             {formData.imageUrl ? (
               ...
               <div className="py-8 space-y-2">
                 ...
                 <p className="text-gray-600 font-medium">Haz clic para subir imagen</p>
               </div>
             )}
             <input 
               type="file" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-3 rounded-lg border border-gray-100">
                  <div className="flex gap-2 items-center">
                      ...
                      <input 
                        ...
                        onChange={handleChange}
                        className="flex-1 p-2 text-sm border border-gray-200 rounded focus:border-brand-red outline-none bg-white text-gray-700"
                  ...
                  <div className="flex gap-2 items-center">
                      ...
                      <input 
                        ...
                        onChange={handleChange}
                        className="flex-1 p-2 text-sm border border-gray-200 rounded focus:border-brand-red outline-none bg-white text-gray-700 font-medium"
                        placeholder="Autor de la foto (Ej: Manolo Herrera)"
                        disabled={isSubmitting}
                      />
                  </div>
              </div>
              ...
              {formData.contentImages?.map((img, idx) => (
                  <div key={idx} className="bg-gray-50 rounded-xl border border-gray-200 p-2 relative group">
                      <div className="relative aspect-video rounded-lg overflow-hidden mb-2">
                          <img src={img.url} alt={`Galería ${idx}`} className="w-full h-full object-cover" />
                          <button
                            type="button"
                      ...
                      <div className="space-y-2">
                        <input 
                            ...
                            className="w-full p-2 text-xs border border-gray-200 rounded focus:border-brand-red outline-none bg-white text-gray-600"
                            placeholder="Pie de foto"
                            disabled={isSubmitting}
                        />
                        <div className="flex items-center gap-1">
                            <CameraIcon size={12} className="text-gray-400" />
                            <input 
                                type="text" 
                                value={img.credit || ''}
                                onChange={(e) => updateGalleryCredit(idx, e.target.value)}
                                className="w-full p-1.5 text-xs border border-gray-200 rounded focus:border-brand-red outline-none bg-white text-gray-600"
                                placeholder="Autor (Crédito)"
                                disabled={isSubmitting}
                            />
                        </div>
                      </div>
        ...
        <div className="space-y-2">
          ...
          <div className={`border rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-brand-red/20 focus-within:border-brand-red bg-white ${isSubmitting ? 'border-gray-200 opacity-70 pointer-events-none' : 'border-gray-200'}`}>
              <div className="flex items-center gap-1 p-2 border-b border-gray-100 bg-gray-50">
                  ...
                  <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => handleFormat('insertUnorderedList')} className="p-2 hover:bg-gray-200 rounded text-gray-700"><List size={18} /></button>
                  <div className="w-px h-6 bg-gray-300 mx-1"></div>
                  <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={handleLink} className="p-2 hover:bg-gray-200 rounded text-gray-700" title="Añadir Enlace"><LinkIcon size={18} /></button>
              </div>
